...
export default function AdminNotificationsPage() {
  ...
  const [section, setSection] = useState("notify"); // notify | floodmap
  ...
  const isFloodMap = section === "floodmap";

  return (
    <div className="flex-1 overflow-y-auto">
      <div className={`${isFloodMap ? "max-w-6xl" : "max-w-3xl"} mx-auto p-4 sm:p-6 lg:p-8 space-y-6`}>
        {/* Header */}
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="material-symbols-outlined filled-icon text-primary text-2xl">
                {isFloodMap ? "flood" : "campaign"}
              </span>
              <h1 className="text-2xl lg:text-3xl font-black tracking-tight">
                {isFloodMap ? t("notifications.floodMapTitle") : t("notifications.sendTitle")}
              </h1>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {isFloodMap ? t("notifications.floodMapSubtitle") : t("notifications.sendSubtitle")}
            </p>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {[
            { key: "notify", icon: "campaign", label: t("notifications.tabSend") },
            { key: "floodmap", icon: "flood", label: t("notifications.tabFloodMap") },
          ].map((tb) => (
            <button
              key={tb.key}
        ...
        {/* Flood Map section */}
        {isFloodMap && <AdminFloodMapEditor />}
